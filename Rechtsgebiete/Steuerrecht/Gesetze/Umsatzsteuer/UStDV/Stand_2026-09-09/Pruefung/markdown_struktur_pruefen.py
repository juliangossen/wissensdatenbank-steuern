...
def verify(config):
    ...
    for k,match in enumerate(anchors):
        ...
        label=meta.findtext('enbez','').replace('(XXXX) ','')
        expected=(joined(group.find('gliederungsbez'))+joined(group.find('gliederungstitel'))) if group is not None else label+joined(meta.find('titel'))
