...
class Converter:
    ...
    def raw(self,e):
        ...
        names={'P':'p','LA':'div','B':'strong','I':'em','SUP':'sup','SUB':'sub','Title':'div','pre':'pre'}
        if t in names:return '<'+names[t]+'>'+m()+'</'+names[t]+'>'
        if t in {'FnArea','Content','DD','DT','F','NB','SP','ABWFORMAT','noindex','kommentar','Ident','TOC','Footnotes','titel'}:return m()
    ...
    def render(self,e,cell=False):
        ...
        if t in {'SUB','SUP'}:return '<'+t.lower()+'>'+self.mixed(e,cell)+'</'+t.lower()+'>'
        if t in {'P','LA','Title'}:
        ...
        if t in {'FnArea','Content','DD','DT','NB','F','SP','ABWFORMAT','noindex','kommentar','titel','TOC','Ident','Footnotes','gliederungstitel','gliederungsbez'}:return self.mixed(e,cell)
