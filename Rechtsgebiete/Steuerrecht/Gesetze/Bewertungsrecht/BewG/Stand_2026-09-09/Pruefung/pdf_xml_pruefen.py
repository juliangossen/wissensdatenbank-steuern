...
def verify_layout(short, name, expected, actual):
    if short=='BewG':
        from tabellen_pdf_pruefen import verify
        return verify(name,expected,actual)
    steps=[]
...
def compare(conf):
    ...
    for i,r in enumerate(records):
        ...
        expected=r['expected']
        if actual!=expected:normtexte[r['name']]={'pdf':actual,'xml':expected}
        entry={'norm_index':i,'name':r['name'],'pdf_chars':len(actual),'xml_chars':len(expected),'equal':actual==expected}
        if actual!=expected:
            sm=difflib.SequenceMatcher(None,expected,actual,autojunk=True)
    ...
    (conf.parent/'PDF_XML_Normtexte.json').write_text(json.dumps(normtexte,ensure_ascii=False,indent=2),encoding='utf-8')
    assert output['alle_normen_vollstaendig_gleich'],(cfg['kuerzel'],'unresolved differences')
