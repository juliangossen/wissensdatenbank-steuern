...
def verify(config):
    ...
    for k,match in enumerate(anchors):
        ...
        expected=(('' if joined(group.find('gliederungsbez')).strip()=='-' else joined(group.find('gliederungsbez')))+joined(group.find('gliederungstitel'))+label+joined(meta.find('titel'))) if group is not None else label+joined(meta.find('titel'))
