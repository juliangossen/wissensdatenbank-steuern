...
def perform(cp):
    ...
    for i,(match,node) in enumerate(zip(matches,root)):
        ...
        if i==0:head=''
        elif g is not None:head=('' if txt(g.find('gliederungsbez')).strip()=='-' else txt(g.find('gliederungsbez')))+txt(g.find('gliederungstitel'))+en+txt(node.find('metadaten/titel'))
